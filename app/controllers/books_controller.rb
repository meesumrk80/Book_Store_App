class BooksController < ApplicationController
  ...
  before_action :load_books, only: [ :index, :show ]
  # GET /books or /books.json
  def index
  if params[:q].present?
    @books = Book.where("title ILIKE ?", "%#{params[:q]}%")
  else
    @books = Book.all
  end
  end
  ...
  private
    ...
    def book_params
      params.require(:book).permit(:title, :author, :price, :stock, :description, :image, category_ids: [])
    end
